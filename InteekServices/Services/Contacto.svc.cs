    ...
    public class Contacto : IContacto
    {
        ...
        public Contracts.Data.Response<String> EnviarMensaje(string nombre, string correo, string telefono, string empresa, string mensaje)
        {
            try {
                ...
                mail.Subject = string.Format("Mensaje de Contacto: {0}", empresa);
                ...
                body += "</div>";
                body += string.Format("<div style='height:100%; width:621px;' align='center'><div style='width:501px; text-align:left;'><p style='width:501px;'><strong>Contacto</strong> {0}</br><strong>Correo</strong> {1}</br><strong>Teléfono</strong> {2}</br><strong>Empresa</strong> {3}</br></br></br><strong>Mensaje</strong></br>{4}</p></div></div>", nombre, correo, telefono, empresa, mensaje);
